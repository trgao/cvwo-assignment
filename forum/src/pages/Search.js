...
function Search() {
    ...
    const baseurl = url;
    const tagurl = 'http://localhost:3000/api/v1/tags?q=' + urlparam.get('q');
    const userurl = 'http://localhost:3000/api/v1/users?q=' + urlparam.get('q');
    const [open, setOpen] = useState(false);
    const [tags, setTags] = useState([]);
    const [users, setUsers] = useState([]);

    useEffect(() => {
        setURL('http://localhost:3000/api/v1/posts?q=' + urlparam.get('q'));
        axios.get(tagurl)
            .then(response => {
